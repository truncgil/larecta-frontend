...
const DropdownUser = () => {
  ...
  const [user, setUser] = useState({
    name: '',
    role: '',
    avatar: UserOne // varsayılan avatar
  });
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          setUser({
            name: userData.name,
            role: userData.role,
            avatar: userData.avatar || UserOne
          });
        } else {
          console.log('Kullanıcı bilgileri alınırken hata:', response.status);
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.ok) {
        // Backend'den başarılı yanıt gelirse
        localStorage.removeItem('token');
